...
from fastapi import FastAPI, Request, HTTPException, BackgroundTasks, WebSocket, WebSocketDisconnect, Response, Header
...
def _assign_ticket() -> str | None:
    """Assign to the receiving device with the fewest open tickets (under TICKET_CAP).
    Returns device_id or None if all devices are at capacity or none are receiving.
    """
    with get_db() as conn:
        devices = [r["id"] for r in conn.execute(
            "SELECT id FROM devices WHERE receiving = 1 ORDER BY id ASC"
        ).fetchall()]
    if not devices:
        return None
    ...
    with get_db() as conn:
        rows = conn.execute(
            ...
            f"AND assigned_to IN ({','.join('?' * len(devices))}) "
            f"GROUP BY assigned_to",
            devices,
        ).fetchall()

    counts = {did: 0 for did in devices}
    for row in rows:
        counts[row["assigned_to"]] = row["cnt"]

    eligible = [(cnt, did) for did, cnt in counts.items() if cnt < TICKET_CAP]
    if not eligible:
        return None
    eligible.sort()
    return eligible[0][1]
...
def _refill_responder(device_id: str):
    """Assign the highest-priority queued ticket directly to a specific device if it has room."""
    with get_db() as conn:
        d = conn.execute(
            "SELECT receiving FROM devices WHERE id = ?", (device_id,)
        ).fetchone()
    if not d or not d["receiving"]:
        return
    with get_db() as conn:
        cnt = conn.execute(
...
class DeviceReceivingUpdate(BaseModel):
    receiving: bool
...
def register_device(body: DeviceRegisterRequest):
    with get_db() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO devices (id, name, receiving, registered_at) VALUES (?, ?, 0, ?)",
            (body.id, body.name, datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
        row = conn.execute("SELECT * FROM devices WHERE id = ?", (body.id,)).fetchone()
    return {"id": row["id"], "name": row["name"], "receiving": bool(row["receiving"])}


@app.get("/devices")
def list_devices():
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM devices ORDER BY registered_at ASC").fetchall()
        device_ids = [r["id"] for r in rows]
        ticket_counts: dict[str, dict] = {did: {"pending": 0, "approved": 0} for did in device_ids}
        if device_ids:
            for row in conn.execute(
                f"SELECT assigned_to, ticket_status, COUNT(*) as cnt FROM messages "
                f"WHERE assigned_to IN ({','.join('?'*len(device_ids))}) AND ticket_status IS NOT NULL "
                f"GROUP BY assigned_to, ticket_status",
                device_ids,
            ).fetchall():
                did = row["assigned_to"]
                if did in ticket_counts:
                    if row["ticket_status"] == "pending_approval":
                        ticket_counts[did]["pending"] = row["cnt"]
                    elif row["ticket_status"] == "approved":
                        ticket_counts[did]["approved"] = row["cnt"]
    return [
        {
            "id": r["id"],
            "name": r["name"],
            "receiving": bool(r["receiving"]),
...
@app.patch("/devices/{device_id}/receiving")
def set_device_receiving(device_id: str, body: DeviceReceivingUpdate):
    with get_db() as conn:
        result = conn.execute(
            "UPDATE devices SET receiving = ? WHERE id = ?",
            (1 if body.receiving else 0, device_id),
        )
        conn.commit()
        if result.rowcount == 0:
            raise HTTPException(status_code=404, detail="Device not found")
    if not body.receiving:
        with get_db() as conn:
            conn.execute(
...
@app.get("/api/tickets")
def get_tickets(x_device_id: str | None = Header(None)):
    if not x_device_id:
...
def update_ticket_extracted(message_id: int, body: ExtractedDataUpdate, x_device_id: str | None = Header(None)):
    if not x_device_id:
        raise HTTPException(status_code=400, detail="X-Device-ID header required")
    with get_db() as conn:
...
def update_ticket_reply(message_id: int, body: TicketReplyUpdate, x_device_id: str | None = Header(None)):
    if not x_device_id:
        raise HTTPException(status_code=400, detail="X-Device-ID header required")
    with get_db() as conn:
...
def approve_ticket(message_id: int, x_device_id: str | None = Header(None)):
    if not x_device_id:
        raise HTTPException(status_code=400, detail="X-Device-ID header required")
    with get_db() as conn:
        ...
        conn.commit()
    _refill_responder(x_device_id)
    _flush_ticket_queue()
    return {"ok": True, "status": "approved"}


@app.post("/api/tickets/{message_id}/claim")
def claim_ticket(message_id: int, x_device_id: str | None = Header(None)):
    if not x_device_id:
        raise HTTPException(status_code=400, detail="X-Device-ID header required")
    with get_db() as conn:
        row = conn.execute(
...
def set_replies_setting(body: RepliesSettingUpdate):
    global _replies_enabled
    with _replies_lock:
